...
const displayName = '[ITL]';

if(node_env === 'development') {
  log.config('console', 'color', 'itl-server', 'TRACE');
} else
if(node_env === 'staging') {
  log.config('file', 'basic', 'itl-server', 'DEBUG');
} else
if(node_env === 'production') {
  log.config('file', 'json',  'itl-server', 'INFO');
}
...
const fork = () => {
  ...
  cps.on('message', mes => {
    ...
    if(mes.name === 'drain') {
      ...
      queue.drain = () => log.info(displayName, 'all itemlookup have been processed.');
      request(queue).subscribe(
        obj => log.debug(displayName, 'finished proceeding itemlookup...', obj)
      , err => log.error(displayName, err.name, err.message, err.stack)
      , ()  => log.info(displayName, 'post itemlookup completed.')
      );
    }
  });
...
const main = () => {
  log.info(displayName, 'start fetch itemlookup server.')
  const queue = Async.queue(worker, cpu_num);
  queue.drain = () => log.info(displayName, 'all itemlookup have been processed.');
